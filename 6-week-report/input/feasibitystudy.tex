...
Web-Octave is a web based software and the 
...
together and relax.
\item Accept inputs from the user.
\item Help M.Tech and Civil Engineer related to OCTAVE.
\item Automatic calculation of modal force and modes.
\item Reduce the time for analysis.
\item Generates the final output in the form of png image.
\item Provide on-line way to analysis so that individual does not have to 
install anything.
\item Save the session of a user.
\end{enumerate}
This project use classes instead of simple dispatch functions.
